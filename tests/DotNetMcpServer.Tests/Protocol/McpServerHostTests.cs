...
public class McpServerHostTests
{
    ...
    /// <summary>
    /// Creates an in-memory pair of JsonRpcStreams (client ↔ server) and runs HandwrittenMcpServer in background.
    /// </summary>
    private sealed class TestEnvironment : IAsyncDisposable
    {
        public JsonRpcConnection ClientRpc { get; }
        private readonly Task _serverTask;
        private readonly CancellationTokenSource _cts;
        private readonly Stream _clientToServer;
        private readonly Stream _serverToClient;

        private TestEnvironment(JsonRpcConnection clientRpc, Task serverTask, CancellationTokenSource cts, Stream clientToServer, Stream serverToClient)
        {
            ClientRpc = clientRpc;
        ...
        public static async Task<TestEnvironment> CreateAsync()
        {
            ...
            var serverRpc = new JsonRpcConnection(clientToServer.ReadStream, serverToClient.WriteStream);
            var clientRpc = new JsonRpcConnection(serverToClient.ReadStream, clientToServer.WriteStream);

            var registry = new ToolRegistry([new FakeTool()]);
            var host = new HandwrittenMcpServer(serverRpc, registry, "TestServer", "1.0.0", "2025-03-26");
